...
protected:
    ...
    bool equal(const AstNode& node) const override {
        assert(isA<const AstRecordInit>(node));
...
public:
    AstSumInit(std::string branch, std::unique_ptr<AstArgument> arg)
            : branch(std::move(branch)), arg(std::move(arg)) {
        assert(this->arg);
    }
    ...
    AstSumInit* clone() const override {
        auto res = new AstSumInit(branch, std::unique_ptr<AstArgument>(arg->clone()));
        res->setSrcLoc(getSrcLoc());
        return res;
    }
...
protected:
    ...
    /** Implements the node comparison for this node type */
    bool equal(const AstNode& node) const override {
        assert(isA<const AstSumInit>(node));
